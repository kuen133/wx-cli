...
/// 查找 db_storage 目录下数据库热文件的最新 mtime。
///
/// macOS 多账号场景下，session.db-wal 通常最能反映当前活跃账号；没有
/// session 热文件时，再 fallback 到递归扫描所有 .db。
fn latest_db_mtime(dir: &Path) -> Option<std::time::SystemTime> {
    ...
    if let Ok(entries) = std::fs::read_dir(&session_dir) {
        for entry in entries.flatten() {
            ...
            if !file_name.starts_with("session.db") {
                continue;
            }
            ...
            latest_session =
                Some(latest_session.map_or(mtime, |cur| if mtime > cur { mtime } else { cur }));
        }
    }
    if latest_session.is_some() {
